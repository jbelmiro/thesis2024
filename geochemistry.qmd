...
p_sp18 <- sp18_long |>
  ...
  geom_label(data = sp18_marks, aes(x = Angle, y = Intensity, label = d), 
            size = 3, fill = "white", label.size = 0.2, vjust = -1, show.legend = FALSE) +
  scale_colour_manual(values = c("lightsteelblue3", "salmon", "steelblue1", "green" ,"orchid","snow4")) +
...
lod_tab <- pxrf_raw |>
  select(-contains("Err")) |>
  select(-pXRF_ID, -Application, -Method, -ElapsedTime, -`Cal Check`) |>
...
# Now, reshape the data into a wide format using pivot_wider
lod_tab_table <- lod_tab_summary |>
  pivot_wider(names_from = Value, values_from = Count, values_fill = list(Count = 0))  # Fill missing values with 0

lod_tab_summary <- lod_tab_long |>
  group_by(Element, Value) |>
  summarise(Count = n(), .groups = "drop")  # Count occurrences of each Value per Element

lod_tab_summary <- lod_tab_summary |>
  count(Element, Value)
...
ggplot(lod_tab_summary, aes(x = Element, y = Count, fill = Value)) +
  ...
  scale_fill_manual(values = c("Success" = "darkseagreen", "Success 0" = "darkseagreen2", "< LOD" = "indianred3")) +
...
boxplots_pxrf <- pxrf_analysis |> 
  select(-Sample_ID, -pXRF_ID) |> 
  filter(!`Formation/Type` %in% c("TW", "T1")) |> # Removing chalcedony
...
boxplots_comb1 <- ggarrange(b3, b10, b9, b2, b4, b7, 
